...
const AboutMeDCCB = () => {
    // Estado para que el color se quede fijo una vez activado
    ...
    useEffect(() => {
        const observer = new IntersectionObserver(
            ([entry]) => {
                // Si la foto entra al menos un 30% en la pantalla, activamos el color
                if (entry.isIntersecting) {
                    setHasBeenRevealed(true);
                }
            },
            { threshold: 0.3 } // 30% de visibilidad
        );
    ...
    return (
        <section className="py-24 border-t border-arch-gray/10 dark:border-white/5">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
                ...
                {/** LADO IZQUIERDO: Imagen con Estética Técnica */}
                <div className="lg:col-span-5 relative group" ref={imageRef}>
                    <div 
                        // Al hacer click o tap, se queda en color para siempre
                        onClick={() => setHasBeenRevealed(true)}
                        className={`relative z-10 overflow-hidden rounded-sm transition-all duration-[1200ms] border border-slate-200 dark:border-white/10 cursor-pointer
                            ${hasBeenRevealed ? 'grayscale-0' : 'grayscale hover:grayscale-0'}
                        `}
                    >
                        <img 
